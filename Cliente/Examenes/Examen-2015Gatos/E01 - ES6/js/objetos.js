...
class Albergue {
	...
	movimientoMascota(dniColaborador, idMascota, tipo, fecha) {
		...
		const colaboradorNoRegistrado = this.colaboradores.filter((elem) => elem.dni == dniColaborador).length == 0;
		const mascotaNoRegistrada = this.mascotas.filter((elem) => elem.idMascota == idMascota).length == 0;
		const mascotaYaRecogida = tipo == "R" && this.movimientos.filter((elem) => elem.mascota.idMascota == idMascota && elem.tipo == "R").length > 0;
		const mascotaYaEntregada = tipo == "E" && this.movimientos.filter((elem) => elem.mascota.idMascota == idMascota && elem.tipo == "E").length > 0;
		const mascotaNoRecogidaPreviamente = tipo == "R" && this.movimientos.filter((elem) => elem.mascota.idMascota == idMascota && elem.tipo == "E").length == 0;
	...
	listadoMascotas() {
		let salida = "<table border='1'><thead><tr><th>Tipo</th><th>ID Mascota</th><th>Peso</th><th>Altura</th><th>Raza</th><th>Colaborador</th><th>Fecha Entrega</th></thead><tbody>";
